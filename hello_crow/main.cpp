...
int main(int argc, char* argv[]) {
  ...
  CROW_ROUTE(app, "/ws")
      ...
      .onmessage([&](crow::websocket::connection& /*conn*/, const string& data,
                     bool is_binary) {
        std::lock_guard<std::mutex> _(mtx);
        for (auto user : users) {
          if (is_binary) {
  ...
  CROW_ROUTE(app, "/chat")
  ([](const request& req, response& res) { sendHtml(res, "chat"); });
  ...
  ([](const request& req, response& res) { sendHtml(res, "about"); });
  ...
  CROW_ROUTE(app, "/rest_test")
      .methods(HTTPMethod::Post, HTTPMethod::Get,
               HTTPMethod::Put)([](const request& req, response& res) {
